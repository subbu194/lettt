...
import { ShoppingCart, X, Plus, Minus, Trash2, ArrowRight, Package, Ticket, AlertCircle } from 'lucide-react';
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 z-50 flex h-full w-full flex-col bg-[var(--color-bg)] shadow-2xl sm:w-[440px]"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-black/10 p-5">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--color-primary-red)]/10">
                  <ShoppingCart className="h-5 w-5 text-[var(--color-primary-red)]" />
                </div>
                <div>
              ...
              {items.length === 0 ? (
                ...
                >
                  <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-black/5">
                    <ShoppingCart className="h-10 w-10 text-[var(--color-muted)]" />
                  </div>
                ...
                <div className="space-y-4">
                      ...
                      >
                        <Card className="p-4">
                          <div className="flex gap-4">
                            {/* Item Image */}
                            <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-xl bg-black/5">
                              {item.image ? (
                                <img src={item.image} alt={item.name} className="h-full w-full object-cover" />
                              ) : (
                                <div className="flex h-full w-full items-center justify-center">
                                  {item.itemType === 'event' ? (
                                    <Ticket className="h-8 w-8 text-[var(--color-muted)]" />
                                  ) : (
                                    <Package className="h-8 w-8 text-[var(--color-muted)]" />
                                  )}
                                </div>
                              )}
                              {/* Type Badge */}
                              <span className="absolute bottom-1 left-1 rounded bg-black/70 px-1.5 py-0.5 text-[10px] font-bold uppercase text-white">
                                {item.itemType || 'item'}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-1 flex-col">
                              ...
                              <div className="mt-auto flex items-center justify-between pt-2">
                                ...
                                <button
                                  ...
                                  className="flex h-8 w-8 items-center justify-center rounded-lg text-[var(--color-muted)] transition-colors hover:bg-[var(--color-primary-red)]/10 hover:text-[var(--color-primary-red)]"
                                >
                  ...
                  {items.length > 0 && (
                    <button
                      onClick={clearCart}
                      className="flex w-full items-center justify-center gap-2 py-2 text-sm font-semibold text-[var(--color-muted)] transition-colors hover:text-[var(--color-primary-red)]"
                    >
                      <Trash2 className="h-4 w-4" />
                      Clear All Items
                    </button>
            ...
            {items.length > 0 && (
              <div className="border-t border-black/10 p-5">
                ...
                {/* Auth Warning */}
                {!isAuthenticated && (
                  <div className="mt-4 flex items-center gap-2 rounded-xl bg-[var(--color-primary-gold)]/20 p-3 text-sm">
                    <AlertCircle className="h-4 w-4 flex-shrink-0 text-[var(--color-primary-red)]" />
                ...
                <button
                  onClick={onClose}
                  className="mt-3 w-full text-center text-sm font-semibold text-[var(--color-muted)] transition-colors hover:text-[var(--color-text)]"
                >
                  Continue Shopping
                </button>
              </div>
      ...
      <Card className="p-8 text-center">
        <div className="mb-4 flex justify-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-black/5">
            <ShoppingCart className="h-8 w-8 text-[var(--color-muted)]" />
          </div>
        </div>
    ...
    <div className="space-y-4">
        ...
        <Card key={item.id} className="p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center justify-between gap-3 sm:justify-end">
              ...
              <button
                className="flex h-10 w-10 items-center justify-center rounded-xl border border-black/10 bg-white text-[var(--color-muted)] hover:border-[var(--color-primary-red)] hover:text-[var(--color-primary-red)]"
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between">
          <div className="text-sm font-semibold tracking-wide text-[var(--color-muted)]">Total</div>
          <div className="text-2xl font-extrabold tracking-tight text-[var(--color-primary-red)]">
            ₹{total.toLocaleString('en-IN')}
          </div>
        </div>
